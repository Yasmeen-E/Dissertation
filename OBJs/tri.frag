...
vec3 sampleNoiseOctaves(vec3 p) {
    ...
    for (int i = 0; i < 3; i++) {
        sum += texture(noiseTex3D, p * frequency).rgb * amplitude;
        frequency *= 2.0;
        amplitude *= 0.5;
    }
...
vec3 GetDistoredNormal(vec3 Normal)
{

}

vec3 PhongDirectionalLighting(float shadow)
{
...
vec3 PhongSnowColour(vec3 Normal)
{
...
void main(){
    ...
    vec3 phongLighting = PhongDirectionalLighting(shadow, distoredNormal);

    vec3 phongCS = PhongDirectionalLighting(shadow, vNormal);
